	...
	render() {
		...
		if (_contentadd!=undefined) {
			_newflow = _contentadd.map((item,index)=>{
				return (
					<div key={index} ref="floatwindow" style={{"position":"absolute","zIndex":"1100",width:_width,height:_height,left:item.left,top:_top}}  className="FloatWindow" onClick={e => {e.stopPropagation()}}>
						<div className="FloatWindowNavigationBar"
							...
							<img onClick={_hiddenSelf} className="FloatWindowNavigationCancleImg" src={_img} alt="x" />
						</div>
